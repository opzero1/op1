...
If `$ARGUMENTS` is empty, default to the current branch diff against `origin/main`.
...
2. Prefer `git diff --name-only origin/main...HEAD` for branch-owned work.
3. Ignore unrelated dirty files outside scope unless the user explicitly includes them.
4. If a file is ambiguous, keep it in scope only when it materially affects the current work.
...
- Return exactly: scope reviewed, issues found, fixes applied, items kept for now, verification evidence, recommended next step.
</output_contract>
...
4. **Helper, type, and constant placement**
...
5. **Styled wrapper justification**
