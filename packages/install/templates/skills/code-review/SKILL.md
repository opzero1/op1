...
Apply these checks for frontend implementation/review work:

- Parse API payloads fail-closed at boundaries (prefer schema parsing such as `zod` over ad-hoc `typeof` chains).
- Critical API fields should not remain optional after parsing (`items/data`, pagination/meta, IDs, URLs).
- Avoid shared async state leaks: loading/disabled states should not unintentionally couple unrelated buttons/views.
- Keep behavior and UX copy consistent across equivalent actions (especially error handling paths).
- Match product spec/staging format exactly when defined (dates, separators, labels, empty states).
- In new/touched components, avoid explicit `(): React.ReactElement` return annotations unless required.
- In touched code, migrate deprecated UI API usage to current props/components.
