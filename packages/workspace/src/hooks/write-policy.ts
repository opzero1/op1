 ...
 *
 * Enforces "Hand Off, Don't Implement" philosophy on the build agent
 * (orchestrator). When the orchestrator tries to use `write` or `edit`
 * tools directly, this hook warns and redirects to subagent handoff via `task`.
 *
 * Override: pass `directEdit: true` in tool args or include `--direct`
 * in the prompt to bypass.
 */

/** Tools that write code and should be handed off */
const WRITE_TOOLS = new Set(["write", "edit"]);

/** Agent types that should hand work off rather than write directly */
const ORCHESTRATOR_AGENTS = new Set(["plan", "build"]);

/**
 * Build the handoff warning message.
 */
...
hand implementation to a specialist agent:
...
Proceeding with the edit — but prefer subagent handoff for production code.
</system-reminder>`;
}
