...
describe("delegation plugin", () => {
	...
	test("honors approval gating for background_cancel when enabled", async () => {
		const root = await mkdtemp(join(tmpdir(), "op1-delegation-approval-"));
		tempRoots.push(root);
		await mkdir(join(root, ".opencode", "workspace"), { recursive: true });
		await Bun.write(
			join(root, ".opencode", "workspace.json"),
			JSON.stringify(
				{
					features: { approvalGate: true },
					approval: {
						mode: "selected",
						tools: ["background_cancel"],
						ttlMs: 60000,
					},
				},
				null,
